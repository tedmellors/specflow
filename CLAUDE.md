...
1) =.specflow/units/core/overview.org= (intent)
2) =.specflow/units/core/architecture.org= (structure)
3) =.specflow/todo.org= (execution plan)
4) unit specs/todos in =.specflow/units/<unit>/= (scoped implementation)
...
│   └── units/
│       ├── core/
│       │   ├── spec.org      # Core specification
│       │   ├── todo.org      # Core tasks
│       │   ├── architecture.org
│       │   └── overview.org
│       └── <unit>/
│           ├── spec.org      # Unit specification (includes scope/constraints)
│           └── todo.org      # Unit tasks
├── units/
│   └── <unit>/
│       ├── src/              # Implementation
