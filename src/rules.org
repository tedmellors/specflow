...
Required sections: Purpose, Scope (In Scope / Out of Scope), Public Interface, Testing Requirements.
Recommended sections (when relevant): Invariants, Error Handling, Dependencies, Non-Goals, Open Questions, Plan Notes.

The =spec-reviewer= agent carries the full checklist with descriptions, quality checks, and section ordering.
...
The =scaffold-writer= agent carries the full format specification.
